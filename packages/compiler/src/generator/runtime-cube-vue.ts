...
export class CubeVueRuntimeGenerator extends RuntimeGenerator {
  ...
  process(): string {
    return this.innerProcess({
      ...
      processPlatformWrapperBase: this.processPlatformWrapperBase.bind(this),
    });
  }
  ...
  private processPlatformWrapperBase(isDiy?: boolean) {
    ...
    if (isDiy) {
      this.writeLine(
        `const options${this.useType ? `: ${optionType}` : ''} = {`,
      );
    } else {
      this.writeLine(
        `const options${
  ...
  private processPlatformWrapper() {
    ...
    // this.writeLine();
    // this.writeLine('return options;');
    this.processPlatformWrapperBase();
  }
}
