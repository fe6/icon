...
export interface ICubeVueGeneratorOptions extends IGeneratorOptions {
  useDefault?: boolean;
  useType?: boolean;
...
export class CubeVueGenerator extends IconGenerator {
  result = '';
  isNewLine = false;
  indentSize = 0;
  constructor(options: ICubeVueGeneratorOptions = defOpts) {
    super(options);
  ...
  process(info: IInfo) {
    ...
    const {
      ...
      rtl,
      importPath,
    } = this.$opts as ICubeVueGeneratorOptions;
